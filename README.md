...
앱 안의 **"설정" 탭**에서 네이버 이메일·앱 비밀번호를 입력·저장하면, "메일 발송" 탭의 **네이버로 실제 발송** 버튼이 그 계정으로 메일을 보냅니다. 이 설정이 실제로 저장되게 하려면 Vercel Marketplace의 무료 저장소(**Upstash Redis**)를 딱 한 번 연결해야 합니다. (Vercel 자체 "KV" 상품은 단종되어 지금은 Marketplace의 Upstash Redis로 대체되었습니다.)

1. Vercel 프로젝트 페이지 → **Storage** 탭 → Marketplace Database Providers에서 **Upstash** 선택 → **Redis** 생성 (무료 플랜 가능).
2. 만든 데이터베이스를 이 프로젝트에 **Connect Project**. (자동으로 `KV_REST_API_URL`, `KV_REST_API_TOKEN` 환경변수가 프로젝트에 추가됩니다 — 직접 입력할 필요 없음)
...
Upstash Redis를 연결하지 않아도, 기존 방식대로 Vercel 프로젝트 Settings → Environment Variables에 `NAVER_EMAIL` / `NAVER_APP_PASSWORD`를 직접 추가하는 방법도 계속 동작합니다 (저장소 설정이 없을 때 이 환경변수를 대신 사용합니다).
